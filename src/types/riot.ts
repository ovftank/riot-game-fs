...
export const RiotErrorCodeSchema = z.enum([
    'invalid_credentials',
    'invalid_code',
    'rate_limited',
    'account_locked',
...
export const SuccessResponseSchema = LoginResponseSchema.refine(
    (data) => data.type === 'success' && data.success !== undefined,
    {
        message: 'Response must be success type with success data',
    }
);

export const ErrorResponseSchema = LoginResponseSchema.refine(
    (data) => data.type === 'error' && data.error !== undefined,
    {
        message: 'Response must be error type with error data',
    }
);

export const MultifactorResponseSchema = LoginResponseSchema.refine(
    (data) => data.type === 'multifactor' && data.multifactor !== undefined,
    {
